...
func RescaleQ(a int, encBase AVRational, stBase AVRational) int {
	return int(C.av_rescale_q(C.int64_t(a), C.AVRational(encBase), C.AVRational(stBase)))
}

func RescaleDelta(inTb AVRational, inTs int, fsTb AVRational, duration int, last *int, outTb AVRational) int {
	return int(C.av_rescale_delta(C.AVRational(inTb), C.int64_t(inTs), C.AVRational(fsTb), C.int(duration), (*C.int64_t)(unsafe.Pointer(&last)), C.AVRational(outTb)))
}
